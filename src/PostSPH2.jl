...
const RhopSearch =
    [0x41; 0x52; 0x52; 0x41; 0x59; 0x04; 0x00; 0x00; 0x00; 0x52; 0x68; 0x6f; 0x70]
const RhopOffset = 12
...
@inline _readel(ft::IOStream, typ::Type{<:SVector{3,T}}) where {T} =
    SVector(_readel(ft, T), _readel(ft, T), _readel(ft, T))
...
readBi4Array(typ::Cat, SeekNull::Bool, Bi4Files::String) =
    readBi4Array(typ, false, [Bi4Files])
readBi4Array(typ::Cat, Bi4Files::String) = readBi4Array(typ, false, [Bi4Files])
function readBi4Array(
    typ::Cat,
    SeekNull::Bool = false,
    Bi4Files::Array{String,1} = _dirFiles(),
)
